            ...
            <Card
                ...
                {isTop && node.parentId !== 0 ? (
                    <CardActions sx={{ display: 'flex', justifyContent: 'center', margin: 0, padding: 0 }}>
                        <IconButton
                            size='small'
                            color='primary'
                            onClick={() => navigationHandler(node.parentId)}>
                            <ExpandLessIcon />
                        </IconButton>
                    </CardActions>
                ) : null}
                <CardContent sx={{ margin: 0, padding: 0 }}>
                    <Grid
                        ...
                        <Grid
                            item
                            sx={{ width: '100%', padding: 1, color: 'white', bgcolor: 'primary.main' }}>
                            <Tooltip
                                title={node.name}
                                placement='top'>
                                <Typography
                ...
                <CardActions sx={{ display: 'flex', justifyContent: 'center', margin: 0, padding: 0 }}>
                    {!isTop && node.children.length > 0 ? (
                        <IconButton
                            ...
                            onClick={() => navigationHandler(node.id)}>
                            <Badge
